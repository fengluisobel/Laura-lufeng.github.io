...
async function exists(path5) {
  return await fs.access(path5, constants.F_OK).then(
    () => true,
    () => false
  );
}
function generateImageID(filename, format2 = "jpeg") {
  return `${createHash("sha256").update(filename).digest("hex").slice(0, 8)}.${format2}`;
}
...
function getIntroAnimFrames() {
  ...
  for (let end = 1; end <= gradientColors.length; end++) {
    ...
    const gradientArr = gradientColors.slice(0, end).map((g) => chalk2.bgHex(g)(" "));
...
var manifestKey = path2.join(cacheDirectory, "manifest.json");
var Cache = class {
  ...
  get(chunk) {
    ...
    const originStats = fs2.statSync(path2.join(root, chunk.name));
    const cacheStats = this.getManifest(cacheKey);
    if (originStats.ctimeMs === cacheStats.ctimeMs) {
      return fs2.readFileSync(cacheKey);
    }
    return null;
  }
  set(chunk, data) {
    ...
    fs2.writeFileSync(
      cacheKey,
      data || fs2.readFileSync(path2.join(this.outputPath, chunk.fileName))
    );
    this.setManifest(cacheKey, fs2.statSync(path2.join(root, chunk.name)));
  }
  getManifest(key) {
  ...
  setManifest(key, value) {
    ...
    fs2.writeFileSync(manifestKey, JSON.stringify(this.manifest), "utf-8");
  }
  hasManifest(key) {
...
function getCacheManifest() {
  ...
  return JSON.parse(fs2.readFileSync(manifestKey, "utf-8")) || {};
}
function getCacheKey(chunk) {
  const hash = digest(`${cacheIdentifier}
${chunk.name}`);
  return path2.join(cacheDirectory, `${hash}`);
}
function existsSync(existsPath) {
  return fs2.existsSync(existsPath);
}
function mkdirSync(mkdirPath) {
  fs2.mkdirSync(mkdirPath, { recursive: true });
}
function digest(str) {
...
async function initSquoosh(config) {
  ...
  const images = files.map(async (filePath) => {
    const fileRootPath = path3.resolve(outputPath, filePath);
    if (options.cache && cache.get(chunks[filePath])) {
      fs3.writeFileSync(fileRootPath, cache.get(chunks[filePath]));
      logger(chalk3.blue(filePath), chalk3.green("\u2728 The file has been cached"));
      return Promise.resolve();
    }
    const start = Date.now();
    const image = imagePool.ingestImage(path3.resolve(outputPath, filePath));
    const oldSize = fs3.lstatSync(fileRootPath).size;
    let newSize = oldSize;
    const ext = path3.extname(path3.resolve(outputPath, filePath)).slice(1) ?? "";
...
async function initSharp(config) {
  ...
  const images = files.map(async (filePath) => {
    const fileRootPath = path4.resolve(outputPath, filePath);
    if (options.cache && cache.get(chunks[filePath])) {
      fs4.writeFileSync(fileRootPath, cache.get(chunks[filePath]));
      logger(chalk4.blue(filePath), chalk4.green("\u2728 The file has been cached"));
      return Promise.resolve();
    }
    const start = Date.now();
    const oldSize = fs4.lstatSync(fileRootPath).size;
    let newSize = oldSize;
    const ext = path4.extname(path4.resolve(outputPath, filePath)).slice(1) ?? "";
    const res = options.conversion.find((item) => `${item.from}`.includes(ext));
    const itemConversion = isTurn && (res == null ? void 0 : res.from) === ext;
    ...
    if (currentType !== void 0) {
      ...
      resultBuffer = await sharp(fileRootPath)[sharpEncodeMap.get(currentType.to)](merge).toBuffer();
    } else {
      const merge = { ...sharpOptions[ext], ...options.compress[ext] };
      resultBuffer = await sharp(fileRootPath)[sharpEncodeMap.get(fileExt)](merge).toBuffer();
    }
...
var Context = class {
  constructor() {
    ...
    this.files = [];
    this.assetPath = [];
    this.filter = createFilter(extSvgRE, [
      /[\\/]node_modules[\\/]/,
      /[\\/]\.git[\\/]/
    ]);
  }
  ...
  async generateBundleHook(bundler) {
    ...
    if (this.imageModulePath.length > 0) {
      ...
      logger(pluginTitle("\u2728"), chalk5.yellow("Successfully"));
    } else {
      console.log(
        chalk5.yellow(
    ...
    spinner.text = chalk5.yellow("Image conversion completed!");
    spinner.succeed();
  }
  /**
   *
  ...
  async generateSquooshBundle(imagePool, item) {
    ...
    const size2 = await fs5.lstat(item);
  ...
  startGenerate() {
    console.log("\n");
    const info = chalk5.gray("Process start with");
    const modeLog = chalk5.magenta(`Mode ${this.config.options.mode}`);
  ...
  async transformHtmlModule() {
    ...
    const html = await fs5.readFile(resolve(process.cwd(), htmlBundlePath));
    ...
    this.config.options.conversion.forEach(async (item) => {
      ...
      await fs5.writeFile(resolve(process.cwd(), htmlBundlePath), newFile);
    });
  }
  ...
  async generateSvgBundle(item) {
    const svgCode = await fs5.readFile(item, "utf8");
...
async function convertToSharp(inputImg, options) {
  ...
  if (currentType !== void 0) {
    ...
    res = await sharp2(inputImg)[sharpEncodeMap.get(currentType.to)](merge).toBuffer();
  } else {
    const merge = {
      ...sharpOptions[ext],
      ...options.compress[ext]
    };
    res = await sharp2(inputImg)[sharpEncodeMap.get(ext)](merge).toBuffer();
  }
  return res;
}
